...
type
  ...
  TBitmaps = class                                                              //Текстуры
   public
    ...
    MayBe:TPNGObject;                                                           //Возможные позиции на поле при расстанвоке (окраска в низкий)
    MayBe2:TPNGObject;                                                          //Окраска в средний
    MayBe3:TPNGObject;                                                          //Окраска в высокий
    ...
    begin
     for y:=1 to 10 do
      begin
       ...
       for k1:=-1 to 1 do
       for k2:=-1 to 1 do
        if (x+k1 <= 10) and (x+k1 > 0) and
           (y+k2 <= 10) and (y+k2 > 0)
        then if APoly[x+k1, y+k2] = aEmpty then Inc(S);
       if S = CSize then
        begin
         Inc(APCount);
         ActualPos[APCount]:=Point(X, Y);
...
begin
 ...
 MayBe:=TPNGObject.Create;
 MayBe2:=TPNGObject.Create;
 MayBe3:=TPNGObject.Create;
     ...
     begin
      for i:=-1 to ShipSize do
       begin
        ...
        if (CheckCell(DPos)) then
         if (not (GetPos(CPoly, DPos)=aEmpty)) then
          if (not (GetPos(CPoly, DPos)=aBusy)) then Exit;
       end;
     end;
    Result:=True;
    Exit;
...
begin
 ...
 Inc(CShips[ShipSize]);
 Direction:=tdLeft;
 if CPoly[SPos.X, SPos.Y] = CPoly[SPos.X+1, SPos.Y] then Direction:=tdRight;
 if CPoly[SPos.X, SPos.Y] = CPoly[SPos.X-1, SPos.Y] then Direction:=tdLeft;
 if CPoly[SPos.X, SPos.Y] = CPoly[SPos.X, SPos.Y+1] then Direction:=tdDown;
 if CPoly[SPos.X, SPos.Y] = CPoly[SPos.X, SPos.Y-1] then Direction:=tdUp;
 case Direction of
  tdLeft:
   begin
    for i:= -1 to ShipSize   do if InsertCheck(CPoly, Point(SPos.X-i, SPos.Y-1)) then CPoly[SPos.X-i, SPos.Y-1]:=aEmpty;
    for i:=  0 to ShipSize-1 do CPoly[SPos.X-i, SPos.Y ]:=aEmpty;
    for i:= -1 to ShipSize   do if InsertCheck(CPoly, Point(SPos.X-i, SPos.Y+1)) then CPoly[SPos.X-i, SPos.Y+1]:=aEmpty;
    if InsertCheck(CPoly, Point(SPos.X+1, SPos.Y)) then CPoly[SPos.X+1, SPos.Y]:=aEmpty;
    if InsertCheck(CPoly, Point(SPos.X-ShipSize, SPos.Y)) then CPoly[SPos.X-ShipSize, SPos.Y]:=aEmpty;
    Exit;
   end;
  ...
  tdUp:
   begin
    for i:= -1 to ShipSize   do if InsertCheck(CPoly, Point(SPos.X-1, SPos.Y-i)) then CPoly[SPos.X-1, SPos.Y-i]:=aEmpty;
    for i:=  0 to ShipSize-1 do CPoly[SPos.X , SPos.Y-i]:=aEmpty;
    for i:= -1 to ShipSize   do if InsertCheck(CPoly, Point(SPos.X+1, SPos.Y-i)) then CPoly[SPos.X+1, SPos.Y-i]:=aEmpty;
    if InsertCheck(CPoly, Point(SPos.X, SPos.Y+1)) then CPoly[SPos.X, SPos.Y+1]:=aEmpty;
    if InsertCheck(CPoly, Point(SPos.X, SPos.Y-ShipSize)) then CPoly[SPos.X, SPos.Y-ShipSize]:=aEmpty;
    Exit;
   end;
...
var i,j:Byte;
begin
 for i:=1 to 10 do for j:=1 to 10 do CPoly[i, j]:=aEmpty;
end;
  ...
  begin
   ...
   if ACol = 11 then
    begin
     ...
     if Ships >= ARow then
      case Ships of
       0..2:Bitmaps.DrawBMP.Canvas.Draw(0, 0, Bitmaps.MayBe3);
       3..5:Bitmaps.DrawBMP.Canvas.Draw(0, 0, Bitmaps.MayBe2);
      else
  ...
  begin
   ...
   if ACol = 0 then
    begin
     ...
     if Ships >= ARow then
      case Ships of
       0..2:Bitmaps.DrawBMP.Canvas.Draw(0, 0, Bitmaps.MayBe);
       3..5:Bitmaps.DrawBMP.Canvas.Draw(0, 0, Bitmaps.MayBe2);
      else
...
var Dir:TDirection;                                                             //Направление корабля (далее ботом не используется)
...
begin
 ...
 if Status <> tsPlay then Exit;                                                 //ЕСли игра остановлена - выходим
 ...
 case Player[SPos.X, SPos.Y] of                                                 //Читаем элемент поля игрока
  ...
  if (PlayerKill[SPos.X, SPos.Y]=aUser) then Exit;                              //Если это помеченный элемент - выходим
...
begin
 ...
 if (NPos.X = 0) or (NPos.Y = 0) then Exit;
 with FormMain do
  begin
   ShowSteps(NPos);
...
begin
 Result:=False;
 if (Status <> tsPlay) or (Lead <> tlComp) then Exit;
 if (SPos.X = 0) or (SPos.Y = 0) then Exit;
 ...
 for i:=-1 to 1 do
  for j:=-1 to 1 do
   begin
    ...
    if (not (Player[SPos.X+i, SPos.Y+j] in [1..4])) and (PlayerKill[SPos.X+i, SPos.Y+j] = aEmpty) then PlayerKill[SPos.X+i, SPos.Y+j]:=aUser;
   end;
...
begin
 ...
 for X:=1 to 3 do
  for Y:=1 to 3 do
   if CompRadar.Radar[X, Y] in [1..4] then Exit;
 Result:=False;
end;
...
begin
    ...
    for Y:=1 to 3 do
     begin
      if CompRadar.Radar[X, Y] in [1..4] then
       begin
        if not ((CompRadar.SPos.X+(X-2) in [1..10]) and (CompRadar.SPos.Y+(Y-2) in [1..10])) then Continue;
 ...
 if (CompShips[3] <> 0) and (CompArms.Bonus >= 4) then                          //Если есть мининосцы и бонусы, то запускаем радар
  begin
   ...
   if ActionRadarComp(GetActualRadarPosition(PlayerKill)) then Exit;
   Exit;
  end;
 if (CompShips[2] <> 0) and (CompArms.Bonus >= 2) then                          //Если есть подлодки и бонусы, то пускаем торпеду
  begin
   ...
   if ActionTorpedoComp(GetActualTorpedoPosition(PlayerKill)) then Exit;
  end;
...
begin
 ...
 if not Settings.ShowCompSteps then Exit;
 repeat
...
begin
 ...
 if not IsNextShoot then                                                        //Если нет рассчитанного хода, то
  begin
   ...
   if (SPos.X = 0) or (SPos.Y = 0) then Exit;                                   //Если, использован какой-то бонус, то выходим
  end
 ...
 if Got then                                                                    //Если бот попал
  begin
   ...
   if CompArms.GoodShoots = 2 then Inc(CompArms.Bonus);                         //Если сделано уже два удачных выстрела, увеличиваем счет бонусов
...
begin
 ...
 if Index mod 2 = 0 then Bitmaps.ListItem.Canvas.Font.Color:=clWhite
 else Bitmaps.ListItem.Canvas.Font.Color:=$00CCCCCC;
...
begin
 ...
 if (MyText = 'без шагов') or
    (MyText = 'непоказывать шаги') or
    (MyText = 'скрыть шаги') or
    (MyText = 'убрать шаги')
 then
  begin
   Settings.ShowCompSteps:=False;
   case Random(2) of
    0:WriteIt('Выключен показ шагов', itGame);
    1, 2:WriteIt('Шаги более не отображаются', itGame);
   end;
   Exit;
  end;
 ...
 then
  begin
   if Game.Status <> tsPlay then Exit;
 ...
 case Key of
  ...
  VK_SPACE: if (Game.MCur.X in [1..10]) and (Game.MCur.Y in [1..10]) then
   begin
    ...
    if (Game.Status = tsPlay) then
     begin
      if (Game.Lead = tlPlayer) then Game.Input(Game.MCur);
     end
...
begin
 ...
 case Game.CurDir of
  True:
   begin
    if Game.CheckInsert(Game.TMP, Point(ACol, ARow), tdRight, Game.GetCurShip) then Bitmaps.DrawBMP3.Canvas.Draw(0, 0, Bitmaps.Maybe)
    else
     case Game.GetCurShip of
      4: if (Game.CheckInsert(Game.TMP, Point(ACol-1, ARow), tdRight, Game.GetCurShip)) or
            (Game.CheckInsert(Game.TMP, Point(ACol-2, ARow), tdRight, Game.GetCurShip)) or
            (Game.CheckInsert(Game.TMP, Point(ACol-3, ARow), tdRight, Game.GetCurShip))
         then Bitmaps.DrawBMP3.Canvas.Draw(0, 0, Bitmaps.Maybe);
      3: if (Game.CheckInsert(Game.TMP, Point(ACol-1, ARow), tdRight, Game.GetCurShip)) or
            (Game.CheckInsert(Game.TMP, Point(ACol-2, ARow), tdRight, Game.GetCurShip))
         then Bitmaps.DrawBMP3.Canvas.Draw(0, 0, Bitmaps.Maybe);
      2: if (Game.CheckInsert(Game.TMP, Point(ACol-1, ARow), tdRight, Game.GetCurShip)) 
         then Bitmaps.DrawBMP3.Canvas.Draw(0, 0, Bitmaps.Maybe);
     end;
  ...
  False:
   begin
    if Game.CheckInsert(Game.TMP, Point(ACol, ARow), tdDown, Game.GetCurShip) then Bitmaps.DrawBMP3.Canvas.Draw(0, 0, Bitmaps.Maybe)
    else
     case Game.GetCurShip of
      4: if (Game.CheckInsert(Game.TMP, Point(ACol, ARow-1), tdDown, Game.GetCurShip)) or
            (Game.CheckInsert(Game.TMP, Point(ACol, ARow-2), tdDown, Game.GetCurShip)) or
            (Game.CheckInsert(Game.TMP, Point(ACol, ARow-3), tdDown, Game.GetCurShip))
         then Bitmaps.DrawBMP3.Canvas.Draw(0, 0, Bitmaps.Maybe);
      3: if (Game.CheckInsert(Game.TMP, Point(ACol, ARow-1), tdDown, Game.GetCurShip)) or
            (Game.CheckInsert(Game.TMP, Point(ACol, ARow-2), tdDown, Game.GetCurShip))
         then Bitmaps.DrawBMP3.Canvas.Draw(0, 0, Bitmaps.Maybe);
      2: if (Game.CheckInsert(Game.TMP, Point(ACol, ARow-1), tdDown, Game.GetCurShip))
         then Bitmaps.DrawBMP3.Canvas.Draw(0, 0, Bitmaps.Maybe);
     end;
   end;
 end;
  ...
  begin
   if not Game.AllSet then
    begin
     ...
     Game.ClearSli;
     case Game.CurDir of
      True:if Game.CheckInsert(Game.TMP, SPos, tdRight, Ship) then
            begin
             Dec(Game.Ships[Ship]);
      ...
      False:if Game.CheckInsert(Game.TMP, SPos, tdDown, Ship) then
             begin
              Dec(Game.Ships[Ship]);
...
begin
 if (not Game.CheckFullPoly) and (not Game.AllSet) then
  begin
   ...
   Game.ClearSli;
   case Game.CurDir of
     True:if Game.CheckInsert(Game.TMP, SPos, tdRight, Ship) then Game.InsertShip(Game.Sli, SPos, tdRight, Ship);
    False:if Game.CheckInsert(Game.TMP, SPos, tdDown , Ship) then Game.InsertShip(Game.Sli, SPos, tdDown , Ship);
   end;
   DrawGridSet.Repaint;
  end
 else if DrawGridSet.Cursor = crDefault then Exit else DrawGridSet.Cursor:=crDefault;
end;
