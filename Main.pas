...
uses
  ...
  Dialogs, StdCtrls, Grids, XPMan, ExtCtrls, HGM.Controls.PanelExt, Buttons, MMSystem,
  ActnList, Vcl.Imaging.pngimage, System.Actions;
...
type
  TFormMain = class(TForm)
    ...
    ButtonRadar: TSpeedButton;
    ButtonTorpedo: TSpeedButton;
    ButtonFire: TSpeedButton;
    DrawGridSet: TDrawGrid;
    TimerSet: TTimer;
    ListBoxHelp: TListBox;
    PanelHelp: TPanelExt;
    ButtonStart: TSpeedButton;
    ButtonAutoTune: TSpeedButton;
    ButtonClearSet: TSpeedButton;
    TimerActions: TTimer;
    PanelResult: TPanelExt;
    LabelLosesV: TLabel;
    LabelWinsV: TLabel;
    ButtonCloseP: TSpeedButton;
    ButtonNewP: TSpeedButton;
    procedure DrawGridPolyDrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; State: TGridDrawState);
    procedure FormCreate(Sender: TObject);
    procedure DrawGridCompDrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; State: TGridDrawState);
    procedure DrawGridCompMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    procedure FormShow(Sender: TObject);
    procedure ListBoxPosDrawItem(Control: TWinControl; Index: Integer; Rect: TRect; State: TOwnerDrawState);
    ...
    procedure DrawGridCompMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
    procedure ListBoxHelpDrawItem(Control: TWinControl; Index: Integer; Rect: TRect; State: TOwnerDrawState);
    procedure DrawGridCompKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
    ...
    procedure DrawGridRadarDrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; State: TGridDrawState);
    ...
    procedure DrawGridSetDrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; State: TGridDrawState);
    procedure DrawGridSetMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
    procedure DrawGridSetMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
    ...
    procedure DrawGridPolyMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
  ...
  THistoryArray = array[1..10, 1..10] of THistShoot;                            //Массив истории выстрелов
  ...
  TSettings = record                                                            //Настройки
    ...
    ShowCompSteps: Boolean;                                                        //Показывать, как комп. делает ход
  end;
  ...
  public
    ...
    Ships: TShips;                                                                //(Расстановка) Кол-во кораблей
    ...
    TMPDir: TArray;                                                               //(Расстановка) Временное поле направлений
    ...
    CompRadar: TCompRadar;                                                        //Данные радара компьютера
   //Обработка действий и вспомогательные процедуры
    ...
    procedure ClearPoly(var CPoly: TArray);                                       //Очистить поле
    ...
    procedure PlayerPast(SPos: TPoint);                                           //Отметить точку на поле бота
    procedure InsertPastForUser(Ship: TPoint);                                    //Оградить убитый корабль комп-а
    procedure InsertShip(var CPoly: TArray; SPos: TPoint; Direction: TDirection; ShipSize: Byte); //Поставить корабль на поле
    procedure DeleteShip(var CPoly: TArray; SPos: TPoint; var CShips: TShips);      //Удалить корабль
    ...
    procedure ActionRadar(SPos: TPoint);                                          //Использовать радар игрока
    ...
    function SetShips: TArray;                                                    //Расставить корабли
    function ActionTorpedoComp(NPos: TPoint): Boolean;                             //Пустить торпеду комп-а
    function ActionRadarComp(SPos: TPoint): Boolean;                               //ИСпользовать радар компьютера
    function CheckCompRadar: Boolean;                                             //Проверить радар компьютера на наличие кораблей
    ...
    function GetActualRadarPosition(APoly: TKillArray): TPoint;                    //Получить актуальные координаты запуска радара
    ...
    function GetPos(CPoly: TArray; SPos: TPoint): Byte;                             //Получить объект из поля
    function CheckPos(CPoly: TArray; SPos: TPoint; var Direction: TDirection; ShipSize: Byte): Boolean; //Проверить можно ли поставить корабль и как поставить
    function CheckInsert(CPoly: TArray; SPos: TPoint; Direction: TDirection; ShipSize: Byte): Boolean; //Проверить на возможность вставки корабля
    function InsertCheck(Poly: TArray; SPos: TPoint): Boolean;                      //Вставить корабль
  ...
  public
    ...
    Wou: TPNGImage;                                                             //Ранил
    Kill: TPNGImage;                                                            //Убил
    UPast: TPNGImage;                                                           //Здесь нет корабля
    ...
    Ships: array[1..4] of TPNGImage;                                            //Корабли (целые)
    Results: array[0..3] of TPNGImage;                                          //Медали
    ShipsPics: array[1..44] of TPNGImage;                                       //Части кораблей (смотреть блокнот)
    Ani1: array[1..9] of TPNGImage;                                             //Анимированный указатель цели
    Fire: array[1..8] of TPNGImage;                                             //Анимированный огонь
    Radar: TPNGImage;                                                           //Радар
    LineRadar: array[1..36] of TPNGImage;                                       //Линия работы радара
    Ready: array[1..3] of TPNGImage;                                            //Готовность
    RadarObject: TPNGImage;                                                     //Объект на радаре (точка)
    MayBe: TPNGImage;                                                           //Возможные позиции на поле при расстанвоке (окраска в низкий)
    MayBe2: TPNGImage;                                                          //Окраска в средний
    MayBe3: TPNGImage;                                                          //Окраска в высокий
...
function GetRect(L, T, R, B: Integer): TRect;                                   //Интерпретация функции Rect для сис. искл.
...
begin
  for i := 1 to 10 do
    for j := 1 to 10 do
    begin
      if TMPDir[i, j] = 0 then
        TMPDir[i, j] := GetInfo(i, j, TMP, TMPDir);
    end;
end;
...
begin
  ...
  Settings.ShowLeadPlayer := False;
  Settings.ShowLeadComp := True;
  Settings.ShowCompSteps := True;
  Wins := 0;
  Loses := 0;
  CurDir := True;
  ClearSli;
...
var
  ActualPos: array[1..200] of TPoint;
  APCount: Byte;
  CSize, S: Byte;
  x, y, k: Byte;
begin
...
function TGame.GetActualRadarPosition(APoly: TKillArray): TPoint;
var
  ActualPos: array[1..200] of TPoint;
  APCount: Byte;
  CSize, S: Byte;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        for k1 := -1 to 1 do
          for k2 := -1 to 1 do
            if (x + k1 <= 10) and (x + k1 > 0) and (y + k2 <= 10) and (y + k2 > 0) then
              if APoly[x + k1, y + k2] = aEmpty then
                Inc(S);
        if S = CSize then
        begin
          Inc(APCount);
...
begin
  ...
  for j := 1 to 10 do
    ...
    begin
      ...
      begin
        if j + k <= 10 then
          if APoly[i, j + k] = aEmpty then
            Inc(S1);
        if i + k <= 10 then
          if APoly[i + k, j] = aEmpty then
            Inc(S2);
      end;
      if (S1 = SSize) then
      begin
        if Result + 1 > 100 then
          Exit;
        Inc(Result);
  ...
  try
    case SoundType of
      stShoot:
        Exit;
      stKill:
        PlaySound(PChar(Path + 'Sound\kill.wav'), 0, TP);
      stWound:
        PlaySound(PChar(Path + 'Sound\wound.wav'), 0, TP);
      stWin:
        PlaySound(PChar(Path + 'Sound\win.wav'), 0, TP);
      stLose:
        PlaySound(PChar(Path + 'Sound\lose' + IntToStr(Random(2) + 1) + '.wav'), 0, TP);
      stPast:
        PlaySound(PChar(Path + 'Sound\past.wav'), 0, TP);
    end;
  except
    NoSound := True;
  end;
...
begin
  ...
  begin
    if Win then
      LabelLosesV.Caption := 'Победа'
    else
      LabelLosesV.Caption := 'Поражение';
    LabelWinsV.Caption := 'Счет: ' + IntToStr(Loses) + ':' + IntToStr(Wins) + ' Очки: ' + IntToStr(LastScore);
    if Loses > Wins then
      LabelWinsV.Hint := 'Не в вашу пользу'
    else if Loses < Wins then
      LabelWinsV.Hint := 'В вашу пользу';
  end;
end;
  ...
  begin
    ...
    case Random(10) of
      ...
      2:
        AText := 'Дьявол';
      3:
        AText := 'О нет, я проиграл';
      4:
        AText := 'Победа за тобой';
      5:
        AText := 'Ты выиграл';
      6:
        AText := 'Поздравляю';
...
procedure TGame.WriteIt(Text: string; InfoType: TInfoType);
  ...
  procedure AWrite;
  begin
    if FormMain.ListBoxPos.Count = 17 then
...
begin
  case InfoType of
    itLeadPlayer:
      if Settings.ShowLeadPlayer then
        AWrite;
    itLeadComp:
      if Settings.ShowLeadComp then
        AWrite;
    itTalk:
      if Settings.ShowCompTalk then
        AWrite;
  else
    AWrite;
  end;
...
begin
  ...
  for i := 1 to 10 do
    for j := 1 to 10 do
      PlayerDirs[i, j] := GetInfo(i, j, Comp, PlayerDirs);
  ...
  case Random(6) of
    0:
      WriteIt(CompName + ': Ходи', itLeadComp);
    1:
      WriteIt(CompName + ': Ходите', itLeadComp);
    2:
      WriteIt(CompName + ': Твой ход', itLeadComp);
    3:
      WriteIt(CompName + ': Вперед!', itLeadComp);
    4:
      WriteIt(CompName + ': GO', itLeadComp);
    5:
      WriteIt(CompName + ': Игра началась, ходи', itLeadComp);
    6:
      WriteIt(CompName + ': Твой залп первый!', itLeadComp);
  end;
...
begin
  ...
  Past := TPNGImage.Create;
  UPast := TPNGImage.Create;
  Wou := TPNGImage.Create;
  Kill := TPNGImage.Create;
  Ships[1] := TPNGImage.Create;
  Ships[2] := TPNGImage.Create;
  Ships[3] := TPNGImage.Create;
  Ships[4] := TPNGImage.Create;
  Radar := TPNGImage.Create;
  RadarObject := TPNGImage.Create;
  MayBe := TPNGImage.Create;
  MayBe2 := TPNGImage.Create;
  MayBe3 := TPNGImage.Create;
  ListItem := TBitmap.Create;
  ListItem.Canvas.Font.Name := 'Segoe UI';
  ListItem.Canvas.Font.Style := [fsBold];
  DrawBMP := TBitmap.Create;
  DrawBMP.Height := 24;
  DrawBMP.Width := 24;
  DrawBMP.Canvas.Font.Name := 'Techno28';  //Segoe UI
  ...
  Results[0] := TPNGImage.Create;
  Results[1] := TPNGImage.Create;
  Results[2] := TPNGImage.Create;
  Results[3] := TPNGImage.Create;
  for i := 1 to 44 do
    ShipsPics[i] := TPNGImage.Create;
  for i := 1 to 9 do
    Ani1[i] := TPNGImage.Create;
  for i := 1 to 8 do
    Fire[i] := TPNGImage.Create;
  for i := 1 to 36 do
    LineRadar[i] := TPNGImage.Create;
  for i := 1 to 3 do
    Ready[i] := TPNGImage.Create;
  inherited;
end;
...
begin
  inherited;
  Exit;
  if (M.Result = htClient) and Need then
    M.Result := htCaption;
end;
...
begin
  ...
  try
    ...
    MayBe.LoadFromResourceName(DLL, 'MAYBE');
    MayBe2.LoadFromResourceName(DLL, 'MAYBE2');
    MayBe3.LoadFromResourceName(DLL, 'MAYBE3');
    for i := 1 to 40 do
      ShipsPics[i].LoadFromResourceName(DLL, 'S' + IntToStr(i));
    for i := 41 to 44 do
      ShipsPics[i].LoadFromResourceName(DLL, 'SHIP' + IntToStr(i - 40));
    for i := 1 to 9 do
      Ani1[i].LoadFromResourceName(DLL, 'A' + IntToStr(i));
    for i := 1 to 8 do
      Fire[i].LoadFromResourceName(DLL, 'F' + IntToStr(i));
    for i := 1 to 36 do
      LineRadar[i].LoadFromResourceName(DLL, 'R' + IntToStr(i));
    ...
    begin
      ...
      PNG.LoadFromResourceName(DLL, 'close');
      ButtonClose.Glyph.Assign(PNG);
      PNG.LoadFromResourceName(DLL, 'hide');
      ButtonHide.Glyph.Assign(PNG);
      PNG.LoadFromResourceName(DLL, 'new');
      ButtonNew.Glyph.Assign(PNG);
      PNG.LoadFromResourceName(DLL, 'user');
      ButtonHelp.Glyph.Assign(PNG);
...
begin
  Result := False;
  if CPoly[SPos.X, SPos.Y] <> aEmpty then
    Exit;
  Used := [];
  Randomize;
  repeat
...
function TGame.CheckInsert(CPoly: TArray; SPos: TPoint; Direction: TDirection; ShipSize: Byte): Boolean;
var
  i, j: Smallint;
  DPos: TPoint;
begin
...
procedure TGame.DeleteShip(var CPoly: TArray; SPos: TPoint; var CShips: TShips);
var
  i: SmallInt;
  ShipSize: Byte;
  Direction: TDirection;
begin
  ShipSize := GetPos(CPoly, SPos);
  Inc(CShips[ShipSize]);
  Direction := tdLeft;
  if CPoly[SPos.X, SPos.Y] = CPoly[SPos.X + 1, SPos.Y] then
    Direction := tdRight;
  if CPoly[SPos.X, SPos.Y] = CPoly[SPos.X - 1, SPos.Y] then
    Direction := tdLeft;
  if CPoly[SPos.X, SPos.Y] = CPoly[SPos.X, SPos.Y + 1] then
    Direction := tdDown;
  if CPoly[SPos.X, SPos.Y] = CPoly[SPos.X, SPos.Y - 1] then
    Direction := tdUp;
  case Direction of
    tdLeft:
      begin
        for i := -1 to ShipSize do
          if InsertCheck(CPoly, Point(SPos.X - i, SPos.Y - 1)) then
            CPoly[SPos.X - i, SPos.Y - 1] := aEmpty;
        for i := 0 to ShipSize - 1 do
          CPoly[SPos.X - i, SPos.Y] := aEmpty;
        for i := -1 to ShipSize do
          if InsertCheck(CPoly, Point(SPos.X - i, SPos.Y + 1)) then
            CPoly[SPos.X - i, SPos.Y + 1] := aEmpty;
        if InsertCheck(CPoly, Point(SPos.X + 1, SPos.Y)) then
          CPoly[SPos.X + 1, SPos.Y] := aEmpty;
        if InsertCheck(CPoly, Point(SPos.X - ShipSize, SPos.Y)) then
          CPoly[SPos.X - ShipSize, SPos.Y] := aEmpty;
        Exit;
      end;
    ...
    tdUp:
      begin
        for i := -1 to ShipSize do
          if InsertCheck(CPoly, Point(SPos.X - 1, SPos.Y - i)) then
            CPoly[SPos.X - 1, SPos.Y - i] := aEmpty;
        for i := 0 to ShipSize - 1 do
          CPoly[SPos.X, SPos.Y - i] := aEmpty;
        for i := -1 to ShipSize do
          if InsertCheck(CPoly, Point(SPos.X + 1, SPos.Y - i)) then
            CPoly[SPos.X + 1, SPos.Y - i] := aEmpty;
        if InsertCheck(CPoly, Point(SPos.X, SPos.Y + 1)) then
          CPoly[SPos.X, SPos.Y + 1] := aEmpty;
        if InsertCheck(CPoly, Point(SPos.X, SPos.Y - ShipSize)) then
          CPoly[SPos.X, SPos.Y - ShipSize] := aEmpty;
        Exit;
      end;
  ...
  case Direction of
    tdLeft:
      begin
        for i := -1 to ShipSize do
          if InsertCheck(CPoly, Point(SPos.X - i, SPos.Y - 1)) then
            CPoly[SPos.X - i, SPos.Y - 1] := aBusy;
        for i := 0 to ShipSize - 1 do
          CPoly[SPos.X - i, SPos.Y] := ShipSize;
        for i := -1 to ShipSize do
          if InsertCheck(CPoly, Point(SPos.X - i, SPos.Y + 1)) then
            CPoly[SPos.X - i, SPos.Y + 1] := aBusy;
        if InsertCheck(CPoly, Point(SPos.X + 1, SPos.Y)) then
          CPoly[SPos.X + 1, SPos.Y] := aBusy;
        if InsertCheck(CPoly, Point(SPos.X - ShipSize, SPos.Y)) then
          CPoly[SPos.X - ShipSize, SPos.Y] := aBusy;
        Exit;
      end;
    ...
    tdUp:
      begin
        for i := -1 to ShipSize do
          if InsertCheck(CPoly, Point(SPos.X - 1, SPos.Y - i)) then
            CPoly[SPos.X - 1, SPos.Y - i] := aBusy;
        for i := 0 to ShipSize - 1 do
          CPoly[SPos.X, SPos.Y - i] := ShipSize;
        for i := -1 to ShipSize do
          if InsertCheck(CPoly, Point(SPos.X + 1, SPos.Y - i)) then
            CPoly[SPos.X + 1, SPos.Y - i] := aBusy;
        if InsertCheck(CPoly, Point(SPos.X, SPos.Y + 1)) then
          CPoly[SPos.X, SPos.Y + 1] := aBusy;
        if InsertCheck(CPoly, Point(SPos.X, SPos.Y - ShipSize)) then
          CPoly[SPos.X, SPos.Y - ShipSize] := aBusy;
        Exit;
      end;
  ...
  repeat                                                                         //Пускаем масштабный цикл расстановки
    ...
    begin
      ...
      begin
        ...
        if OutExcept = 3000000 then
          Break;                                        //Если исчерпаны попытки, то прерываем тек. итерацию
...
procedure TGame.ClearPoly(var CPoly: TArray);
var
  i, j: Byte;
begin
  for i := 1 to 10 do
...
procedure TFormMain.DrawGridPolyDrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; State: TGridDrawState);
var
  W, H, Ships: Byte;
...
begin
  ...
  begin
    ...
    if (Game.LastShoots.Last.X = ACol) and (Game.LastShoots.Last.Y = ARow) then
      Bitmaps.DrawBMP.Canvas.Draw(0, 0, Bitmaps.Ani1[Bitmaps.CurAni]);
  ...
  begin
    ...
    if ACol = 11 then
    begin
      Ships := Game.PlayerShips[1] + Game.PlayerShips[2] + Game.PlayerShips[3] + Game.PlayerShips[4];
      if Ships >= ARow then
        case Ships of
          0..2:
            Bitmaps.DrawBMP.Canvas.Draw(0, 0, Bitmaps.MayBe3);
          3..5:
            Bitmaps.DrawBMP.Canvas.Draw(0, 0, Bitmaps.MayBe2);
        else
          Bitmaps.DrawBMP.Canvas.Draw(0, 0, Bitmaps.MayBe);
        end;
    end;
    DrawGridPoly.Canvas.StretchDraw(Rect, Bitmaps.DrawBMP);
    Exit;
  ...
  if (Game.LastShoots.Last.X = ACol) and (Game.LastShoots.Last.Y = ARow) then
    Bitmaps.DrawBMP.Canvas.Draw(0, 0, Bitmaps.Ani1[Bitmaps.CurAni]);
  if Game.Player[ACol, ARow] in [1..4] then
    Bitmaps.DrawBMP.Canvas.Draw(0, 0, Bitmaps.ShipsPics[Game.CompDirs[ACol, ARow]]);

  case Game.PlayerKill[ACol, ARow] of
    aPast:
      Bitmaps.DrawBMP.Canvas.Draw(0, 0, Bitmaps.Past);
    aWound:
      Bitmaps.DrawBMP.Canvas.Draw(0, 0, Bitmaps.Fire[Bitmaps.CurFire]);
    aKill:
      Bitmaps.DrawBMP.Canvas.Draw(0, 0, Bitmaps.Fire[Bitmaps.CurFire]);
    aUser:
      Bitmaps.DrawBMP.Canvas.Draw(0, 0, Bitmaps.UPast);
  end;
  DrawGridPoly.Canvas.StretchDraw(Rect, Bitmaps.DrawBMP);
end;
...
procedure TFormMain.DrawGridCompDrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; State: TGridDrawState);
var
  W, H, Ships: Byte;
...
begin
  ...
  begin
    Bitmaps.DrawBMP.Canvas.CopyRect(GetRect(0, 0, 24, 24), Bitmaps.FormBrush.Canvas, GetCurRect);
    if (Game.MCur.X = ACol) and (Game.MCur.Y = ARow) then
      Bitmaps.DrawBMP.Canvas.Draw(0, 0, Bitmaps.Ani1[Bitmaps.CurAni]);
  ...
  begin
    ...
    if ACol = 0 then
    begin
      Ships := Game.CompShips[1] + Game.CompShips[2] + Game.CompShips[3] + Game.CompShips[4];
      if Ships >= ARow then
        case Ships of
          0..2:
            Bitmaps.DrawBMP.Canvas.Draw(0, 0, Bitmaps.MayBe);
  ...
  case Direction of
    tdLeft:
      for i := 0 to SSize - 1 do
        if Poly[SPos.X - i, SPos.Y] <> aWound then
          Exit;
    tdRight:
      for i := 0 to SSize - 1 do
        if Poly[SPos.X + i, SPos.Y] <> aWound then
    ...
    tdUp:
      for i := 0 to SSize - 1 do
        if Poly[SPos.X, SPos.Y - i] <> aWound then
          Exit;
    tdDown:
      for i := 0 to SSize - 1 do
        if Poly[SPos.X, SPos.Y + i] <> aWound then
...
begin
  case Direction of
    tdLeft:
      for i := 0 to SSize - 1 do
        Poly[SPos.X - i, SPos.Y] := aKill;
    tdRight:
      for i := 0 to SSize - 1 do
        Poly[SPos.X + i, SPos.Y] := aKill;
    tdUp:
      for i := 0 to SSize - 1 do
        Poly[SPos.X, SPos.Y - i] := aKill;
    tdDown:
      for i := 0 to SSize - 1 do
        Poly[SPos.X, SPos.Y + i] := aKill;
  end;
  case Whose of
    tlPlayer:
      Game.PlayerShips[SSize] := Game.PlayerShips[SSize] - 1;
    tlComp:
      Game.CompShips[SSize] := Game.CompShips[SSize] - 1;
  end;
end;
...
begin
  Result := False;
  Got := False;
  if not Game.CheckCell(SPos) then
    Exit;
  case Comp[SPos.X, SPos.Y] of
    1..4:
      begin
        if (CompKill[SPos.X, SPos.Y] = aWound) or (CompKill[SPos.X, SPos.Y] = aKill) or (CompKill[SPos.X, SPos.Y] = aUser) then
          Exit;
    ...
    if (CompKill[SPos.X, SPos.Y] = aUser) then
      Exit;
    if (CompKill[SPos.X, SPos.Y] = aPast) then
      Exit;
...
var
  Dir: TDirection;                                                             //Направление корабля (далее ботом не используется)
  StartPos: TPoint;                                                            //Начало корабля (далее ботом не используется)
  Size: Byte;                                                                  //Размер корабля (далее ботом не используется)
    //Эти переменные необходимы только тогда, когда корабль уничтожен этим
    //ударом для разметки как убитый

begin
  Result := False;                                                                 //Удар небыл сделан
  if Status <> tsPlay then
    Exit;                                                 //ЕСли игра остановлена - выходим
  Got := False;                                                                    //Попадания небыло
  case Player[SPos.X, SPos.Y] of                                                 //Читаем элемент поля игрока
    1..4:
      begin
        if (PlayerKill[SPos.X, SPos.Y] = aWound) or                                   //Если позиция уже помечена (ранен),
          (PlayerKill[SPos.X, SPos.Y] = aKill) or                                    //убит или
          (PlayerKill[SPos.X, SPos.Y] = aUser)                                       //флаг
          then
          Exit;                                                                  //Выходим
        PlayerKill[SPos.X, SPos.Y] := aWound;                                         //Ставим ранение кораблю игрока
        GetShipPos(SPos, Dir, StartPos, Player);                                    //Получаем полные координаты корабля игрока
        Size := Player[SPos.X, SPos.Y];                                               //Получаем длину корабля игрока
        if ShipIsAllWound(StartPos, Dir, PlayerKill, Size) then                     //Если ранение было критическим - корабль убит
        begin
          KillShip(StartPos, Dir, PlayerKill, Size, tlPlayer);                      //Флаг корабля - убит
          DrawAll(tlComp);                                                          //Перерисовываем поле
        end;
        Result := True;                                                               //Удар был сделан успешно
        Got := True;                                                                  //Было попадание
        Exit;                                                                       //Выходим
      end;
  else                                                                           //Если элемент поля игрока не корабль
    if (PlayerKill[SPos.X, SPos.Y] = aUser) then
      Exit;                              //Если это помеченный элемент - выходим
    PlayerKill[SPos.X, SPos.Y] := aPast;                                            //Ставим - мимо
    Result := True;                                                                 //Удар был сделан успешно
  end;
end;
...
begin
  with FormMain do
  begin
    if not PlayerShoot(SPos, Got) then
      Exit;
    GoSound(stShoot);
    Waiting;
...
function TGame.ActionTorpedoComp(NPos: TPoint): Boolean;
var
  Got: Boolean;
  SPos: TPoint;
  i: Byte;
begin
  Result := False;
  if (Status <> tsPlay) or (Lead <> tlComp) then
    Exit;
  if (NPos.X = 0) or (NPos.Y = 0) then
    Exit;
  with FormMain do
  begin
    ShowSteps(NPos);
...
function TGame.ActionRadarComp(SPos: TPoint): Boolean;
var
  i, j: SmallInt;
begin
  Result := False;
  if (Status <> tsPlay) or (Lead <> tlComp) then
    Exit;
  if (SPos.X = 0) or (SPos.Y = 0) then
    Exit;
  WriteIt(CompName + ': Использую радар', itLeadComp);
  Result := True;
  CompRadar.SPos := SPos;
  CompRadar.Use := False;
  for i := -1 to 1 do
    for j := -1 to 1 do
    begin
      CompRadar.Radar[2 + i, 2 + j] := Player[SPos.X + i, SPos.Y + j];
      if (not (Player[SPos.X + i, SPos.Y + j] in [1..4])) and (PlayerKill[SPos.X + i, SPos.Y + j] = aEmpty) then
        PlayerKill[SPos.X + i, SPos.Y + j] := aUser;
    end;
  CompRadar.Use := CheckCompRadar;
  DrawAll(tlComp);
  Lead := tlPlayer;
 //Мы получаем массив с объектами
 //и в сооствествии с ним, указываем пустые точки на поле,
 //а так же, есть ли корабли в поле действия радара (Use)
end;
...
var
  ...
  TText: string;
begin
  if (Status <> tsPlay) or (Lead <> tlPlayer) then
    Exit;
  if (not (NPos.X in [1..10])) or (not (NPos.Y in [1..10])) then
    Exit;
  ...
  if PlayerArms.Bonus >= 2 then
    Dec(PlayerArms.Bonus, 2)
  else
    Exit;
...
begin
  if (Status <> tsPlay) or (Lead <> tlPlayer) then
    Exit;
  if (not (SPos.X in [1..10])) or (not (SPos.Y in [1..10])) then
    Exit;
  ...
  if PlayerArms.Bonus >= 4 then
    Dec(PlayerArms.Bonus, 4)
  else
    Exit;
  for i := -1 to 1 do
    for j := -1 to 1 do
...
procedure TFormMain.DrawGridCompMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  SPos: TPoint;
  ACol, ARow: Integer;
begin
  ...
  if (ACol = 0) or (ARow = 0) or (ARow = 11) or (ACol = 11) then
    Exit;
  case Button of
    mbLeft:
      begin
...
begin
  ...
  if (CheckCell(Point(SPos.X, SPos.Y - 1))) and (Poly[SPos.X, SPos.Y - 1] = Ship) then
    Direction := tdUp
  else if (CheckCell(Point(SPos.X, SPos.Y + 1))) and (Poly[SPos.X, SPos.Y + 1] = Ship) then
    Direction := tdDown
  else if (CheckCell(Point(SPos.X - 1, SPos.Y))) and (Poly[SPos.X - 1, SPos.Y] = Ship) then
    Direction := tdLeft
  else if (CheckCell(Point(SPos.X + 1, SPos.Y))) and (Poly[SPos.X + 1, SPos.Y] = Ship) then
    Direction := tdRight
  else
  ...
  case Direction of
    tdLeft:
      begin
        if SPos.X <> 10 then
        begin
          for i := SPos.X to 10 do
            if Poly[i + 1, SPos.Y] <> Ship then
              Break;
          StartPos.X := i;
        end
        else
    ...
    tdUp:
      begin
        if SPos.Y <> 10 then
        begin
          for i := SPos.Y to 10 do
            if Poly[SPos.X, i + 1] <> Ship then
              Break;
          StartPos.Y := i;
        end
        else
...
begin
  ...
  for i := 1 to 10 do
    for j := 1 to 10 do
      if Player[i, j] in Ships then
        if PlayerKill[i, j] <> aKill then
          Exit;
  Result := True;
end;
...
begin
  ...
  for i := 1 to 10 do
    for j := 1 to 10 do
      if Comp[i, j] in Ships then
        if CompKill[i, j] <> aKill then
          Exit;
  Result := True;
end;
...
begin
  ...
  repeat
    if (not UseLastDir) or (LastDir in Used) then
      ...
      FirstDir := Ord(LastDir);
    case TDirection(FirstDir) of
      tdLeft:
        begin
          DPos := Point(LastShoot.X - 1, LastShoot.Y);
          while PlayerKill[DPos.X, DPos.Y] = aWound do
            Dec(DPos.X);
...
var
  ...
  begin
    Result := (SPos.X >= 1) and (SPos.X <= 10) and (SPos.Y >= 1) and (SPos.Y <= 10) and (PlayerKill[SPos.X, SPos.Y] = aEmpty);
  end;

begin
  ShipSize := Player[Ship.X, Ship.Y];
  if not Game.GetShipPos(Ship, Dir, SPos, Player) then
    Exit;
  case Dir of
    tdLeft:
      begin
        for i := -1 to ShipSize do
          if CheckPoint(Point(SPos.X - i, SPos.Y - 1)) then
            PlayerKill[SPos.X - i, SPos.Y - 1] := aUser;
        for i := -1 to ShipSize do
          if CheckPoint(Point(SPos.X - i, SPos.Y + 1)) then
            PlayerKill[SPos.X - i, SPos.Y + 1] := aUser;
        if CheckPoint(Point(SPos.X + 1, SPos.Y)) then
          PlayerKill[SPos.X + 1, SPos.Y] := aUser;
        if CheckPoint(Point(SPos.X - ShipSize, SPos.Y)) then
          PlayerKill[SPos.X - ShipSize, SPos.Y] := aUser;
        Exit;
      end;
    ...
    tdUp:
      begin
        for i := -1 to ShipSize do
          if CheckPoint(Point(SPos.X - 1, SPos.Y - i)) then
            PlayerKill[SPos.X - 1, SPos.Y - i] := aUser;
        for i := -1 to ShipSize do
          if CheckPoint(Point(SPos.X + 1, SPos.Y - i)) then
            PlayerKill[SPos.X + 1, SPos.Y - i] := aUser;
        if CheckPoint(Point(SPos.X, SPos.Y + 1)) then
          PlayerKill[SPos.X, SPos.Y + 1] := aUser;
        if CheckPoint(Point(SPos.X, SPos.Y - ShipSize)) then
          PlayerKill[SPos.X, SPos.Y - ShipSize] := aUser;
        Exit;
      end;
...
var
  ...
  function CheckPoint(SPos: TPoint): Boolean;
  begin
    Result := (SPos.X >= 1) and (SPos.X <= 10) and (SPos.Y >= 1) and (SPos.Y <= 10) and (CompKill[SPos.X, SPos.Y] = aEmpty);
  end;

begin
  ShipSize := Comp[Ship.X, Ship.Y];
  if not GetShipPos(Ship, Dir, SPos, Comp) then
    Exit;
  case Dir of
    tdLeft:
      begin
        for i := -1 to ShipSize do
          if CheckPoint(Point(SPos.X - i, SPos.Y - 1)) then
            CompKill[SPos.X - i, SPos.Y - 1] := aUser;
        for i := -1 to ShipSize do
          if CheckPoint(Point(SPos.X - i, SPos.Y + 1)) then
            CompKill[SPos.X - i, SPos.Y + 1] := aUser;
        if CheckPoint(Point(SPos.X + 1, SPos.Y)) then
          CompKill[SPos.X + 1, SPos.Y] := aUser;
        if CheckPoint(Point(SPos.X - ShipSize, SPos.Y)) then
          CompKill[SPos.X - ShipSize, SPos.Y] := aUser;
        Exit;
      end;
    ...
    tdUp:
      begin
        for i := -1 to ShipSize do
          if CheckPoint(Point(SPos.X - 1, SPos.Y - i)) then
            CompKill[SPos.X - 1, SPos.Y - i] := aUser;
        for i := -1 to ShipSize do
          if CheckPoint(Point(SPos.X + 1, SPos.Y - i)) then
            CompKill[SPos.X + 1, SPos.Y - i] := aUser;
        if CheckPoint(Point(SPos.X, SPos.Y + 1)) then
          CompKill[SPos.X, SPos.Y + 1] := aUser;
        if CheckPoint(Point(SPos.X, SPos.Y - ShipSize)) then
          CompKill[SPos.X, SPos.Y - ShipSize] := aUser;
        Exit;
      end;
...
function TGame.CheckCompRadar: Boolean;
var
  X, Y: Byte;
begin
  Result := True;
  for X := 1 to 3 do
    for Y := 1 to 3 do
      if CompRadar.Radar[X, Y] in [1..4] then
        Exit;
  Result := False;
end;
...
begin
  ...
  begin
    for X := 1 to 3 do
      ...
      begin
        if CompRadar.Radar[X, Y] in [1..4] then
        begin
          if not ((CompRadar.SPos.X + (X - 2) in [1..10]) and (CompRadar.SPos.Y + (Y - 2) in [1..10])) then
            Continue;
          if PlayerKill[CompRadar.SPos.X + (X - 2), CompRadar.SPos.Y + (Y - 2)] <> aEmpty then
          begin
            CompRadar.Radar[X, Y] := aEmpty;
            Continue;
          end;
          CompRadar.Radar[X, Y] := aEmpty;
          CompRadar.Use := CheckCompRadar;
          Result := Point(CompRadar.SPos.X + (X - 2), CompRadar.SPos.Y + (Y - 2));
          Exit;
        end;
      end;
  end;
  if (CompShips[3] <> 0) and (CompArms.Bonus >= 4) then                          //Если есть мининосцы и бонусы, то запускаем радар
  begin
    Dec(CompArms.Bonus, 4);
    if ActionRadarComp(GetActualRadarPosition(PlayerKill)) then
      Exit;
    Exit;
  end;
  ...
  begin
   ...
   //Result:=GetActualTorpedoPosition(PlayerKill);
    if ActionTorpedoComp(GetActualTorpedoPosition(PlayerKill)) then
      Exit;
  end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          case LastDir of
            tdDown:
              Result.Y := Result.Y + Random(2);
            tdRight:
              Result.X := Result.X + Random(2);
          end;
          UseLastDir := True;
          Break;
        end;
...
begin
 ...
 //LastShoots.Last - старая позиция
  if not Settings.ShowCompSteps then
    Exit;
  repeat
    if LastShoots.Last.X < NewPos.X then
      Inc(LastShoots.Last.X);
    if LastShoots.Last.X > NewPos.X then
      Dec(LastShoots.Last.X);
    if LastShoots.Last.Y < NewPos.Y then
      Inc(LastShoots.Last.Y);
    if LastShoots.Last.Y > NewPos.Y then
      Dec(LastShoots.Last.Y);
    Waiting(500);
    DrawAll(tlComp);
  until (NewPos.X = LastShoots.Last.X) and (NewPos.Y = LastShoots.Last.Y);
end;
...
begin
  ...
  begin
    SPos := GetFirePosition;                                                       //Рассчитываем позицию для удара
    ItNS := False;                                                                 //Флаг "Это не серийный удар"
    if (SPos.X = 0) or (SPos.Y = 0) then
      Exit;                                   //Если, использован какой-то бонус, то выходим
  end
  else                                                                           //Если нужно продолжить серию ударов
  begin
    SPos := GetNextShoot(CompLastShoot);                                           //Рассчитываем удар относительно предыдущих
    ItNS := True;                                                                  //Флаг "Это серийный удар"
  end;
 //Выбрали позицию удара
  ShowSteps(SPos);                                                               //Показываем перемещение курсора до позиции
  LastShoots.Last := SPos;                                                         //Заносим выбранную позицию удара в память
  Waiting;                                                                       //Ждем (для предания объемности игре)
  WriteIt(CompName + ': ' + RuChars[SPos.X] + '-' + IntToStr(SPos.Y), itLeadComp);       //Пишем от бота в чат (необходимо изменить)
  CompShoot(SPos, Got);                                                          //Делаем низкоуровневый удар
  DrawAll(tlComp);                                                               //Принудительная перерисовка
  if Got then                                                                    //Если бот попал
  begin
    Inc(CompArms.GoodShoots);                                                    //Увеличиваем счет бота (для бонусов)
    if CompArms.GoodShoots = 2 then
      Inc(CompArms.Bonus);                         //Если сделано уже два удачных выстрела, увеличиваем счет бонусов
    if PlayerKill[SPos.X, SPos.Y] = aKill then                                     //Если корабль был убит
    begin
      GoSound(stKill);                                                           //Звук убитого корабля
      Waiting;                                                                   //Ждем (для предания объемности игре)
      WriteIt(PlayerName + ': Убил', itLeadPlayer);                                //Пишем в чат
      InsertPast(SPos);                                                          //Расставляем флаги вокруг корабля
      DrawAll(tlComp);                                                           //Принудительная перерисовка
     //Сбрасываем данные о последнем выстреле
      LastShoots.ShipSize := 0;                                                    //Размер корабля
      LastShoots.Use := False;                                                     //Использование
      IsNextShoot := False;                                                        //Флаг "Не рассчитывать относительно последнего выстрела"
      ItNS := False;                                                               //Флаг "Это не серийный удар"
      CompUsedDir := [];                                                           //Использованые направления выстрелов по кораблю
      if CheckPlayer then                                                        //Проверяем поле игрока, если все корабли убиты
      begin
        Status := tsEnd;                                                           //Игра окончена
        OnCompWin;                                                               //Бот выиграл
        Exit;                                                                    //Выходим
      end;
    end
    else                                                                         //Если корабль не убит, а лишь ранен
    begin
      GoSound(stWound);                                                          //Звук ранения корабля
      Waiting;                                                                   //Ждем (для предания объемности игре)
      WriteIt(PlayerName + ': Попал', itLeadPlayer);                               //Чат
      DrawAll(tlComp);                                                           //Принудительная перерисовка
      Inc(LastShoots.ShipSize);                                                  //Увеличиваем размер корабля в который попали (т.к. он небыл убит)
      LastShoots.Shoots[LastShoots.ShipSize] := SPos;                              //Добавляем удычный выстрел в серию ударов
      LastShoots.Use := False;                                                     //
      UseLastDir := ItNS;
      IsNextShoot := True;
    end;
...
procedure TFormMain.ListBoxPosDrawItem(Control: TWinControl; Index: Integer; Rect: TRect; State: TOwnerDrawState);
var
  DRect, WRect: TRect;
begin
  DRect.Left := 0;
  ...
  Bitmaps.ListItem.Canvas.Brush.Style := bsClear;
  if Index mod 2 = 0 then
    Bitmaps.ListItem.Canvas.Font.Color := clWhite
  else
    Bitmaps.ListItem.Canvas.Font.Color := $00CCCCCC;
 //Bitmaps.ListItem.Canvas.TextOut(Rect.Left+3, Rect.Top, ListBoxPos.Items.Strings[index]);
  Bitmaps.ListItem.Canvas.TextOut(3, 0, ListBoxPos.Items.Strings[Index]);
...
var
  SPos: TPoint;                                                                //в противном случае передача на обработку сообщения
  Num: string;
  IntNum: Byte;
begin
  Result := False;
  ...
  if Text[2] = '-' then
    Num := Copy(Text, 3, Length(Text) - (Length(Text) - 3))
  else
    Num := Copy(Text, 2, Length(Text) - (Length(Text) - 2));
  try
    IntNum := StrToInt(Num);
  except
    Exit;
  end;
  if (IntNum <= 0) or (IntNum > 10) then
    Exit;
  SPos.Y := IntNum;
  Game.Input(SPos);
  Result := True;
...
var
  ...
  begin
    Result := False;
    for l := 0 to NotCensorshipList.Count - 1 do
      if AWord = NotCensorshipList.Strings[l] then
        Exit;
    Result := True;
  end;
  ...
  begin
    ...
    repeat
      Inc(i);
      if (AText[i] = ' ') or (AText[i] = ',') or (AText[i] = '.') then
      begin
        if not NormWord(Copy(AText, iOld, i - iOld)) then
          Exit;
        iOld := i + 1;
        Continue;
      end;
      if i = Length(Text) then
      begin
        if not NormWord(Copy(AText, iOld, i - iOld + 1)) then
          Exit;
        iOld := i;
        Continue;
      end;
...
begin
  ...
  if (MyText = 'шаги') or (MyText = 'показывать шаги') then
  begin
    Settings.ShowCompSteps := True;
    WriteIt('Включен показ шагов', itGame);
    Exit;
  end;
  if (MyText = 'без шагов') or (MyText = 'непоказывать шаги') or (MyText = 'скрыть шаги') or (MyText = 'убрать шаги') then
  begin
    Settings.ShowCompSteps := False;
    case Random(2) of
      0:
        WriteIt('Выключен показ шагов', itGame);
      1, 2:
        WriteIt('Шаги более не отображаются', itGame);
    end;
    Exit;
  end;
  if (MyText = 'новая игра') or                                                  //Начать новую игру
    (MyText = 'начать заново') or (MyText = 'старт новой') or (MyText = 'играть') or (MyText = 'расставить корабли') or (MyText = 'расстановка кораблей') or (MyText = 'заново играть') or (MyText = 'новый бой') or (MyText = 'старт') then
  begin
  ...
  else if (Pos('меня зовут', MyText) <> 0) then                                       //Представление имени
  begin
  ...
  else if (Pos('я ', MyText) <> 0) then                                               //Представление имени
    if (MyText[Pos('я ', MyText) - 1] = '') or (MyText[Pos('я ', MyText) - 1] = ' ') then
    begin
  ...
  if (Pos('привет', MyText) <> 0) or                                             //Приветствие
    (Pos('здравствуй', MyText) <> 0) or (Pos('приветствие', MyText) <> 0) or (Pos('хай', MyText) <> 0) or (Pos('hi', MyText) <> 0) or (Pos('даров', MyText) <> 0) or (Pos('здарова', MyText) <> 0) or (Pos('здаров', MyText) <> 0) then
  begin
    case Random(5) of
      0:
        tmpstr := 'Привет';
    ...
    begin
      if PlayerName <> '' then
        Answer := tmpstr + ', ' + PlayerName;
    end
    else
      Answer := tmpstr + ',я ' + CompName + ', приятно познакомится';
    FormMain.TimerSpeak.Enabled := True;
    A := True;
  end;
  if (Pos('пока', MyText) = 1) or                                                //Прощание - выход из игры
    (Pos('до свидания', MyText) = 1) or (Pos('до встречи', MyText) = 1) or (Pos('выход', MyText) = 1) or (Pos('exit', MyText) = 1) or (Pos('quit', MyText) = 1) or (Pos('закрыть', MyText) = 1) or (Pos('close', MyText) = 1) or (Pos('bye', MyText) = 1) then
  begin
    Answer := 'До встречи';
    FormMain.TimerSpeak.Enabled := True;
    A := True;
  ...
  if (MyText = 'ходи') or                                                        //Передача хода, пропуск хода
    (MyText = 'ты ходишь') or (MyText = 'ты ходишь первый') or (MyText = 'ты ходи') or (MyText = 'твой ход') or (MyText = 'давай ходи') then
  begin
    if Game.Status <> tsPlay then
      Exit;
    Lead := tlComp;
  ...
  if (Pos('sleep=', MyText) <> 0) or                                             //Установка времени задержки между ходами бота
    (Pos('время=', MyText) <> 0) or (Pos('время ', MyText) <> 0) then
  begin
    try
...
var
  VM: string;
begin
  if TimerGo.Interval = 30000 then
    TimerGo.Interval := 60000
  else if TimerGo.Interval = 60000 then
    TimerGo.Interval := 70000
  else if TimerGo.Interval = 70000 then
    TimerGo.Interval := 300000
  else
    TimerGo.Enabled := False;
  Randomize;
  case Game.Status of
    ...
    tsEnd:
      begin
        case Random(6) of
          0:
            VM := 'Счет ' + IntToStr(Game.Wins) + ':' + IntToStr(Game.Loses) + ' давай же изменим его';
          1:
            VM := 'Ну расставляй';
          2:
            VM := 'Расставляй бегом';
          3:
            VM := 'Хочу играть';
          4:
            VM := 'Играть, играть и ещё раз играть';
          5:
            VM := 'Я уже готов, а ты?';
          6:
            VM := 'Ну же, начинай';
...
begin
  PanelHelp.Canvas.Brush.Style := bsClear;
  PanelHelp.Canvas.Font.Name := 'Segoe UI';
  ...
  PanelHelp.Canvas.Draw(10, 50, Bitmaps.Fire[1]);
  PanelHelp.Canvas.TextOut(40, 55, ' - Убил');
  PanelHelp.Canvas.Draw(10, 75, Bitmaps.Fire[1]);
...
begin
  if Game.Score = 0 then
    PanelResult.Canvas.Draw(0, 0, Bitmaps.Results[0]);
  if Game.Score >= 1 then
    PanelResult.Canvas.Draw(0, 0, Bitmaps.Results[1]);
  if Game.Score >= 2 then
    PanelResult.Canvas.Draw(0, 0, Bitmaps.Results[2]);
  if Game.Score >= 3 then
    PanelResult.Canvas.Draw(0, 0, Bitmaps.Results[3]);
end;
...
procedure TFormMain.DrawGridCompMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
var
  ACol, ARow: Integer;
begin
  DrawGridComp.MouseToCell(X, Y, ACol, ARow);
  ...
  begin
    if DrawGridComp.Cursor = crDefault then
      Exit
    else
      DrawGridComp.Cursor := crDefault;
  end
  else
  begin
    LabelXP.Caption := RuChars[ACol] + '-' + IntToStr(ARow);
    if DrawGridComp.Cursor = 1 then
      Exit
    else
...
procedure TFormMain.ListBoxHelpDrawItem(Control: TWinControl; Index: Integer; Rect: TRect; State: TOwnerDrawState);
var
  DRect, WRect: TRect;
begin
  DRect.Left := 0;
...
procedure TFormMain.DrawGridCompKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  case Key of
    VK_LEFT:
      if Game.MCur.X - 1 < 0 then
        Exit
      else
      begin
        Dec(Game.MCur.X);
...
begin
  if Game.Status = tsPlay then
  begin
    if MessageBox(Handle, 'Начать новую игру? Текущая будет прервана!', '', MB_ICONQUESTION or MB_YESNO) <> ID_YES then
      Exit;
  end;
  if Game.Status = tsPlacement then
    Exit;
  Game.ClearAll;
  Game.ClearList;
  Game.TuneForPlacement;
...
begin
  with Bitmaps do
  begin
    if CurAni = Length(Bitmaps.Ani1) then
      DoUp1 := False;
    if CurAni = 1 then
      DoUp1 := True;
    if DoUp1 then
    ...
    if CurFire = Length(Bitmaps.Fire) then
      DoUp2 := False;
    if CurFire = 1 then
      DoUp2 := True;
    if DoUp2 then
    ...
    Inc(CurLine);
    if CurLine > Length(Bitmaps.LineRadar) then
      CurLine := 1;
    if LineCL < 5 then
    begin
      if CurLine <= 10 then
...
procedure TFormMain.DrawGridRadarDrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; State: TGridDrawState);
var
  x, y: Byte;
...
begin
    ...
    for y := 1 to 3 do
      ...
      begin
       ...
       //Если прошла 2 четверть (точки, [1:1], [1:2], [2:1], [2:2]). Позиция линии (not in [1..10])
          if (x <= 2) and (y <= 2) and (Bitmaps.LineCL > 1) then
            Bitmaps.DrawBMP2.Canvas.Draw((16 * x) + 26 + Random(2) - 1, (16 * y) + 14 + Random(2) - 1, Bitmaps.RadarObject);
       //Если прошла 3 четверть (точки, [1:2], [1:3], [2:2], [2:3]). Позиция линии (not in [11..20])
          if (x <= 2) and (y > 2) and (Bitmaps.LineCL > 2) then
            Bitmaps.DrawBMP2.Canvas.Draw((16 * x) + 26 + Random(2) - 1, (16 * y) + 14 + Random(2) - 1, Bitmaps.RadarObject);
       //Если прошла 4 четверть (точки, [2:2], [2:3], [3:2], [3:3]). Позиция линии (not in [21..30])
          if (x > 2) and (y > 2) and (Bitmaps.LineCL > 3) then
            Bitmaps.DrawBMP2.Canvas.Draw((16 * x) + 26 + Random(2) - 1, (16 * y) + 14 + Random(2) - 1, Bitmaps.RadarObject);
       //Если прошла 1 четверть (точки, [2:1], [2:2], [3:1], [3:2]). Позиция линии (not in [31..36])
          if (x > 2) and (y <= 2) and (Bitmaps.LineCL > 4) then
            Bitmaps.DrawBMP2.Canvas.Draw((16 * x) + 26 + Random(2) - 1, (16 * y) + 14 + Random(2) - 1, Bitmaps.RadarObject);
        end
        else
          Bitmaps.DrawBMP2.Canvas.Draw((16 * x) + 26, (16 * y) + 14, Bitmaps.RadarObject);
      end;
  ...
  begin
    ...
    case y + 1 of
      1, 2:
        Bitmaps.DrawBMP2.Canvas.Draw(114, 98 - (24 * y), Bitmaps.Ready[1]);
      3, 4:
        Bitmaps.DrawBMP2.Canvas.Draw(114, 98 - (24 * y), Bitmaps.Ready[2]);
      5:
        Bitmaps.DrawBMP2.Canvas.Draw(114, 98 - (24 * y), Bitmaps.Ready[3]);
    end;
  end;
...
procedure TFormMain.DrawGridSetDrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; State: TGridDrawState);
var
  W, H: Byte;
...
begin
  ...
  case Game.TMP[ACol, ARow] of
    1..4:
      Bitmaps.DrawBMP3.Canvas.Draw(0, 0, Bitmaps.ShipsPics[Game.TMPDir[ACol, ARow]]);
  end;
  case Game.Sli[ACol, ARow] of
    1..4:
      Bitmaps.DrawBMP3.Canvas.Draw(0, 0, Bitmaps.Ships[3]);
    5:
      Bitmaps.DrawBMP3.Canvas.Draw(0, 0, Bitmaps.Ships[1]);
  end;
  if not Game.AllSet then
    case Game.CurDir of
      True:
        begin
          if Game.CheckInsert(Game.TMP, Point(ACol, ARow), tdRight, Game.GetCurShip) then
            Bitmaps.DrawBMP3.Canvas.Draw(0, 0, Bitmaps.Maybe)
          else
            case Game.GetCurShip of
              4:
                if (Game.CheckInsert(Game.TMP, Point(ACol - 1, ARow), tdRight, Game.GetCurShip)) or (Game.CheckInsert(Game.TMP, Point(ACol - 2, ARow), tdRight, Game.GetCurShip)) or (Game.CheckInsert(Game.TMP, Point(ACol - 3, ARow), tdRight, Game.GetCurShip)) then
                  Bitmaps.DrawBMP3.Canvas.Draw(0, 0, Bitmaps.Maybe);
              3:
                if (Game.CheckInsert(Game.TMP, Point(ACol - 1, ARow), tdRight, Game.GetCurShip)) or (Game.CheckInsert(Game.TMP, Point(ACol - 2, ARow), tdRight, Game.GetCurShip)) then
                  Bitmaps.DrawBMP3.Canvas.Draw(0, 0, Bitmaps.Maybe);
              2:
                if (Game.CheckInsert(Game.TMP, Point(ACol - 1, ARow), tdRight, Game.GetCurShip)) then
                  Bitmaps.DrawBMP3.Canvas.Draw(0, 0, Bitmaps.Maybe);
            end;
      ...
      False:
        begin
          if Game.CheckInsert(Game.TMP, Point(ACol, ARow), tdDown, Game.GetCurShip) then
            Bitmaps.DrawBMP3.Canvas.Draw(0, 0, Bitmaps.Maybe)
          else
            case Game.GetCurShip of
              4:
                if (Game.CheckInsert(Game.TMP, Point(ACol, ARow - 1), tdDown, Game.GetCurShip)) or (Game.CheckInsert(Game.TMP, Point(ACol, ARow - 2), tdDown, Game.GetCurShip)) or (Game.CheckInsert(Game.TMP, Point(ACol, ARow - 3), tdDown, Game.GetCurShip)) then
                  Bitmaps.DrawBMP3.Canvas.Draw(0, 0, Bitmaps.Maybe);
              3:
                if (Game.CheckInsert(Game.TMP, Point(ACol, ARow - 1), tdDown, Game.GetCurShip)) or (Game.CheckInsert(Game.TMP, Point(ACol, ARow - 2), tdDown, Game.GetCurShip)) then
                  Bitmaps.DrawBMP3.Canvas.Draw(0, 0, Bitmaps.Maybe);
              2:
                if (Game.CheckInsert(Game.TMP, Point(ACol, ARow - 1), tdDown, Game.GetCurShip)) then
                  Bitmaps.DrawBMP3.Canvas.Draw(0, 0, Bitmaps.Maybe);
            end;
        end;
    end;
...
procedure TFormMain.DrawGridSetMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  SPos: TPoint;
  ACol, ARow: Integer;
  Ship: Byte;
begin
  ...
  begin
    ...
    begin
      ...
      if (ARow >= 11) or (ACol >= 11) or (ARow <= 0) or (ACol <= 0) then
        Exit;
      ...
      case Game.CurDir of
        True:
          if Game.CheckInsert(Game.TMP, SPos, tdRight, Ship) then
          begin
            Dec(Game.Ships[Ship]);
            Game.InsertShip(Game.TMP, SPos, tdRight, Ship);
          end
          else
          begin
            Game.InsertShip(Game.Sli, SPos, tdRight, Ship);
            Game.WriteIt('Так ставить нельзя', itGame);
          end;
        False:
          if Game.CheckInsert(Game.TMP, SPos, tdDown, Ship) then
          begin
            Dec(Game.Ships[Ship]);
            Game.InsertShip(Game.TMP, SPos, tdDown, Ship);
          end
          else
          begin
            Game.InsertShip(Game.Sli, SPos, tdDown, Ship);
            Game.WriteIt('Так ставить нельзя', itGame);
          end;
      end;
      Game.UpdateShips;
      if Game.CheckFullPoly then
      begin
        Game.AllSet := True;
        Game.WriteIt('Все корабли расставлены', itGame);
        Game.WriteIt('Для начала игры нажмите "Старт"', itGame);
      end;
      DrawGridSet.Repaint;
    end;
  end;
...
procedure TFormMain.DrawGridSetMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
var
  SPos: TPoint;
  ACol, ARow: Integer;
  Ship: Byte;
begin
  ...
  begin
    if DrawGridSet.Cursor <> 2 then
      DrawGridSet.Cursor := 2;
    DrawGridSet.MouseToCell(X, Y, ACol, ARow);
    if (Game.OldC = ACol) and (Game.OldR = ARow) and (Game.CurDir = Game.OldDir) then
      Exit;
    if not ((ACol = 0) or (ARow = 0) or (ACol = 11) or (ARow = 11)) then
      LabelXP.Caption := RuChars[ACol] + '-' + IntToStr(ARow);
  ...
  else if DrawGridSet.Cursor = crDefault then
    Exit
  else
    DrawGridSet.Cursor := crDefault;
end;
...
begin
  Game.AClear;
  Game.ClearSli;
  Game.WriteIt('Все корабли убраны', itGame);
end;

procedure TFormMain.ButtonAutoTuneClick(Sender: TObject);
begin
  Game.AClear;
  Game.TMP := Game.SetShips;
  Game.UpdateShips;
  Game.ClearSli;
  Game.AllSet := True;
  DrawGridSet.Repaint;
...
procedure TFormMain.DrawGridPolyMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
var
  ACol, ARow: Integer;
begin
