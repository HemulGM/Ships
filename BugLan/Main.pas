...
uses
  ...
  Buttons, MMSystem, ActnList, ScktComp;
...
type
  ...
  public
   ...
   procedure Send(SendPack:TSendPack);                                          //Отправить сетевому игроку информацию
   ...
   constructor Create;
  published
   property Lead:TLead read ALead write SetLead;
  end;
  TBitmaps = class                                                              //Текстуры
   public
    ...
    MayBe:TPNGObject;                                                           //Возможные позиции на поле при расстанвоке
  ...
  TYour = (tyClient, tyServer);
  TInfo = record
   PlayerName:string;
   Arrays:array[0..1] of TArray;
  end;
...
procedure TGame.Send(SendPack:TSendPack);
//var Buf:string;
begin
 LastSend:=SendPack;
 case Your of
  tyServer:
   case SendPack of
    spRepeat:
     begin
      FormMain.ServerSocket.Socket.Connections[0].SendText(tsRepeat);
      Sleep(100);
     end;
    spShoot:
     begin
      FormMain.ServerSocket.Socket.Connections[0].SendText(tsShoot);
      Sleep(100);
      FormMain.ServerSocket.Socket.Connections[0].SendBuf(MCur, SizeOf(TPoint));
      Sleep(100);
     end;
    spInfo:
     begin
      FormMain.ServerSocket.Socket.Connections[0].SendText(tsInfo);
      Sleep(100);
      Info.PlayerName:=PlayerName;
      Info.Arrays[0]:=Player;
      Info.Arrays[1]:=CompDirs;
      FormMain.ServerSocket.Socket.Connections[0].SendBuf(Info, SizeOf(TInfo));
      Sleep(100);
     end;
    spLead:
     begin
      FormMain.ServerSocket.Socket.Connections[0].SendText(tsLead);
      Sleep(100);
     end;
   end;
  tyClient:
   case SendPack of
    spRepeat:
     begin
      FormMain.ClientSocket.Socket.SendText(tsRepeat);
      Sleep(100);
     end;
    spShoot:
     begin
      FormMain.ClientSocket.Socket.SendText(tsShoot);
      Sleep(100);
      FormMain.ClientSocket.Socket.SendBuf(MCur, SizeOf(TPoint));
      Sleep(100);
     end;
    spInfo:
     begin
      FormMain.ClientSocket.Socket.SendText(tsInfo);
      Sleep(100);
      FormMain.ClientSocket.Socket.SendBuf(Info, SizeOf(TInfo));
      Sleep(100);
     end;
    spLead:
     begin
      FormMain.ClientSocket.Socket.SendText(tsLead);
      Sleep(100);
     end;
   end;
 end;
...
procedure TGame.SetLead(l:TLead);
begin
 ALead:=l;
 //if IsLanGame and (l = tlComp) then Send(spLead);
end;
...
begin
 ...
 if not IsLanGame then
  ...
  begin
   Send(spInfo);
   if not InfoIsReceived then WriteIt('Ожидаем игрока...', itGame);
   while not InfoIsReceived do
    begin
     if StopWait then
      begin
       TuneForPlacement;
 ...
 case Your of
  tyClient:Lead:=tlComp;
  tyServer:Lead:=tlPlayer;
 end;
...
begin
 ...
 if (MyText = 'lan') then FormLanSet.Show;
 if (MyText = 'шаги') or
...
begin
 ...
 if not IsLanGame then TimerCompShoot.Enabled:=True
 else Game.Send(spLead);
end;
...
begin
 ...
 if not IsLanGame then Game.ClearAll;
...
procedure TFormMain.ClientSocketRead(Sender: TObject;
  Socket: TCustomWinSocket);
begin
 ListBoxPos.Items.Add('Пришли данные');
 if RetBuf = '' then
  begin
   RetBuf:=Socket.ReceiveText;
   ListBoxPos.Items.Add(RetBuf);
   Exit;
  end;
 if RetBuf = tsShoot then
  begin
   Socket.ReceiveBuf(PLPos, SizeOf(TPoint));
   Game.DoCompShoot;
   RetBuf:='';
   Exit;
  end;
 if RetBuf = tsInfo then
  begin
   try
    Socket.ReceiveBuf(Info, SizeOf(TInfo));
   except
    begin
     Game.Send(spRepeat);
     Exit;
   ...
   InfoIsReceived:=True;
   Game.Comp:=Info.Arrays[0];
   Game.PlayerDirs:=Info.Arrays[1];
   Game.CompName:=Info.PlayerName;
   ListBoxPos.Items.Add('Получена инф. для начала игры');
   RetBuf:='';
   Exit;
  end;
 if RetBuf = tsLead then Game.Lead:=tlPlayer;
end;
