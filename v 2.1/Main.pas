...
type
  ...
  TStatus = (tsPlay, tsEnd);                                                    //Статус игры
  ...
  TArms = record                                                                //Вооружение
   Torpedo:SmallInt;                                                             //Торпеды
   Radar:SmallInt;                                                               //Радар
  end;
  ...
  public
   ...
   function CheckComp:Boolean;                                                  //Проверить не выиграл ли комп.
   function CheckPlayer:Boolean;                                                //Проверить не выиграл ли игрок
  ...
  TBitmaps = class                                                              //Текстуры
   public
    ...
    Radar:TPNGObject;
    LineRadar:array[1..36] of TPNGObject;
    DrawBMP2:TBitmap;
    RadarObject:TPNGObject;
    DoUp1:Boolean;
    DoUp2:Boolean;
    DoUp3:Boolean;
    CurAni:Byte;
...
var Got:Boolean;
 ...
 i:Byte;
begin
 if (Game.Status <> tsPlay) or (Game.Lead <> tlPlayer) then Exit;
...
begin
 ...
 repeat
  if LastShoots.Last.X < SPos.X then Inc(LastShoots.Last.X);
  if LastShoots.Last.X > SPos.X then Dec(LastShoots.Last.X);
  if LastShoots.Last.Y < SPos.Y then Inc(LastShoots.Last.Y);
  if LastShoots.Last.Y > SPos.Y then Dec(LastShoots.Last.Y);
  Waiting(500);
  DrawAll(tlComp);
 until (SPos.X = LastShoots.Last.X) and
       (SPos.Y = LastShoots.Last.Y);
end;
...
begin
 ...
 if (MyText = 'начать случайную игру') then                                     //Начать случайную новую игру
  begin
   ClearAll;
   FormShipPoly.ButtonAutoClick(nil);
   Player:=SetShip.TMP;
   CompDirs:=TMPDir;
   New;
 ...
 case Key of
  ...
  VK_SPACE: if (Game.MCur.X in [1..10]) and (Game.MCur.Y in [1..10]) then
   begin
    ...
    if (Game.Status = tsPlay) then
     begin
      if (Game.Lead=tlPlayer) then Game.Input(Game.MCur) else Exit;
     end
